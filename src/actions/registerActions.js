...
import {
    ...
    NAME_CHANGEDREG, DIRECCION_CHANGEDREG, INFO_USER, PHONE_CHANGEDRED
} from './types';
...
export const loginUserreg = ({regis}) => {
    ...
    return (dispatch) => {
        ...
        firebase.auth().createUserWithEmailAndPassword(email, password)
            .then(user => loginUserSuccessreg(dispatch, user))
            .then(()=>{firebase.database().ref().child('usersP/')
                .push({email, names, direccion, phone})
                .then(()=>{
